...
import com.xiaoxin.womenclass.adapter.TestRecyclerViewAdapter;
...
public class RecyclerViewFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private void InitData() {
        BmobQuery<SomeNews> query = new BmobQuery<SomeNews>();
        query.findObjects(getActivity(), new FindListener<SomeNews>() {
            @Override
            public void onSuccess(List<SomeNews> someNewses) {
                mAdapter = new RecyclerViewMaterialAdapter(new TestRecyclerViewAdapter(someNewses,getActivity()));
